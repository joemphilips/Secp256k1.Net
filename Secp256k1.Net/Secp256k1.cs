...
namespace Secp256k1Net
{
    ...
    public unsafe class Secp256k1 : IDisposable
    {
        ...
        private readonly Lazy<secp256k1_ecdsa_recoverable_signature_parse_compact> secp256k1_ecdsa_recoverable_signature_parse_compact;
        private readonly Lazy<secp256k1_ecdsa_recoverable_signature_serialize_compact> secp256k1_ecdsa_recoverable_signature_serialize_compact;
        private readonly Lazy<secp256k1_ecdsa_sign_recoverable> secp256k1_ecdsa_sign_recoverable;
        private readonly Lazy<secp256k1_ecdsa_sign> secp256k1_ecdsa_sign;
        private readonly Lazy<secp256k1_ecdsa_recover> secp256k1_ecdsa_recover;
        private readonly Lazy<secp256k1_ecdsa_signature_normalize> secp256k1_ecdsa_signature_normalize;
        private readonly Lazy<secp256k1_ecdsa_signature_parse_der> secp256k1_ecdsa_signature_parse_der;
        private readonly Lazy<secp256k1_ecdsa_verify> secp256k1_ecdsa_verify;
        private readonly Lazy<secp256k1_ecdh> secp256k1_ecdh;
        private readonly Lazy<secp256k1_ec_pubkey_tweak_add> secp256k1_ec_pubkey_tweak_add;
        private readonly Lazy<secp256k1_ec_privkey_tweak_add> secp256k1_ec_privkey_tweak_add;
        private readonly Lazy<secp256k1_ec_pubkey_tweak_mul> secp256k1_ec_pubkey_tweak_mul;
        private readonly Lazy<secp256k1_ec_privkey_tweak_mul> secp256k1_ec_privkey_tweak_mul;
        ...
        public bool Recover(Span<byte> publicKeyOutput, Span<byte> signature, Span<byte> message)
        {
            ...
            fixed (byte* publicKeyPtr = publicKeyOutput)
        ...
        public bool SecretKeyVerify(Span<byte> secretKey)
        {
            ...
            fixed (byte* privKeyPtr = secretKey)
            {
        ...
        public bool PublicKeyCreate(Span<byte> publicKeyOutput, Span<byte> privateKeyInput)
        {
            ...
            fixed (byte* privKeyPtr = privateKeyInput)
            {
        ...
        public bool RecoverableSignatureParseCompact(Span<byte> signatureOutput, Span<byte> compactSignature, int recoveryID)
        {
            ...
            fixed (byte* sigPtr = signatureOutput)
            fixed (byte* inputPtr = compactSignature)
            {
        ...
        public bool SignRecoverable(Span<byte> signatureOutput, Span<byte> messageHash, Span<byte> secretKey)
        {
            ...
            fixed (byte* sigPtr = signatureOutput)
            fixed (byte* msgPtr = messageHash)
            fixed (byte* secPtr = secretKey.Slice(secretKey.Length - 32))
            {
        ...
        public bool RecoverableSignatureSerializeCompact(Span<byte> compactSignatureOutput, out int recoveryID, Span<byte> signature)
        {
            ...
            int recID = 0;
            fixed (byte* compactSigPtr = compactSignatureOutput)
            fixed (byte* sigPtr = signature)
            {
        ...
        public bool SignatureNormalize(Span<byte> normalizedSignatureOutput, Span<byte> signatureInput)
        {
            ...
            fixed (byte* outPtr = normalizedSignatureOutput)
            fixed (byte* intPtr = signatureInput)
            {
        ...
        public bool Ecdh(Span<byte> resultOutput, Span<byte> publicKey, Span<byte> privateKey)
        {
            ...
            fixed (byte* resPtr = resultOutput)
            fixed (byte* pubPtr = publicKey)
            fixed (byte* privPtr = privateKey)
            {
